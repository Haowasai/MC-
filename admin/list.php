        ...
        <ul class="nav navbar-nav navbar-right">
          ...
          <li class="active">
            ...
            <ul class="dropdown-menu">
			  ...
			  <li class="active"><a href="./list.php">QQ列表</a></li>
...
if(isset($_GET['qq'])) {
	...
	$con='QQ '.$_GET['qq'].' 共有 <b>'.$gls.'</b> 个QQ用户';
}else{
	$gls=$DB->count("SELECT count(*) from black_list WHERE 1");
	$sql=" 1";
	$con='QQ后台共有 <b>'.$gls.'</b> 个QQ用户';
}
...
?>
      <div class="table-responsive">
        <table class="table table-striped">
          <thead><tr><th>ID</th><th>ＱＱ</th><th>添加时间</th></tr></thead>
          <tbody>
<?php
$rs=$DB->query("SELECT * FROM black_list WHERE{$sql} order by id desc limit $pageu,$pagesize");
while($res = $DB->fetch($rs))
{
echo '<tr><td>'.$res['id'].'</td><td>'.$res['qq'].'</td><td>'.$res['date'].'</td><td></td><td><a href="./edit.php?my=update&id='.$res['id'].'" class="btn btn-xs btn-info">修改</a> <a href="./edit.php?my=del&id='.$res['id'].'" class="btn btn-xs btn-danger" onclick="return confirm(\'你确实要解除这个QQ用户吗？\');">删除</a></td></tr>';
}
?>
          </tbody>
        </table>
